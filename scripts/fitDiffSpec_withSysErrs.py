...
def loadData():
    x,y,e = numpy.loadtxt('./keck/differenceSpectrum_withSysErrs.txt').T
    mask = x<9000
...
def errorbars(p):
    ...
    err = np.sqrt(err**2 + e**2) # add errors in quadrature
    m1 = (x>4085) & (x<4115)
    m2 = (x>4290) & (x<4320)
    m3 = (x>4845) & (x<4875)
    m4 = (x>5560) & (x<5590)
...
def ln_prior(p):
    ...
    prior = Prior('log_uniform',0.0001,20.0)
...
if __name__ == "__main__":
    ...
    #diffSpec  = scale(y,diffSpec)*diffSpec
    err,mask = errorbars(pars)
